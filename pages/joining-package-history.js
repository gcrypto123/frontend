...
const JoiningPackageHistory = ({rowData}) =>{ 
    ...
    const Columns = useMemo(()=>{
        if(formatedRowData?.length>0){
            ...
            return ['#','MemberID','Wallet Address','Package','Type','Date'];
        }
    },[])
    return <><Container fluid className="px-6 py-6">
        <Row>
            <Team colConfig={Columns} rowData={formatedRowData} tableName="Join Package History" />
        </Row>
    </Container></>
...
export default JoiningPackageHistory;